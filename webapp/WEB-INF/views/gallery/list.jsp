			...
			<div id="content">
				...
				<div id="gallery">
					...
					<c:if test="${not empty authUser}">
						...
						<ul id="viewArea">
							<c:forEach var="gVo" items="${galleryVo}">
								<!-- 이미지반복영역 -->
								<li>
									<div data-no="${gVo.no }" class="view">
										<img class="imgItem" src="${pageContext.request.contextPath }/upload/${gVo.saveName}">
										<div class="imgWriter">
											작성자: <strong>${gVo.name }</strong>
										</div>
									</div>
								</li>
								<!-- 이미지반복영역 -->
			...
			<div class="modal-content">
				...
				<form method="" action="">
					<div class="modal-footer">
						...
						<c:if test="${authUser.no == 7}">
						<button type="button" class="btn btn-danger" id="btnDel">삭제</button>
						</c:if>
						<input id="galleryNo" type="text" name="no" value="">
					</div>
